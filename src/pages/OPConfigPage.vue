...
export default defineComponent({
  ...
  methods: {
    async fetchDiscovery() {
      ...
      try {
        ...
        this.form.scopesSupported = Array.isArray(d.scopes_supported) ? d.scopes_supported.join(' ') : (d.scopes_supported || '')
  ...
  mounted() {
    if (!this.isNew) {
      const record = this.providers.getById(this.id)
      if (record) {
